...
describe("SignerAgent", () => {
  describe("construction", () => {
    it("cannot be constructed directly", () => {
      expect(() => new (SignerAgent as any)({})).toThrow(
  ...
  describe("call", () => {
    ...
    it("rejects if content map does not match request", async () => {
      ...
      await expect(agent.call(CANISTER_ID, CALL_FIELDS)).rejects.toThrow(
        SignerAgentError,
      );
    });

    it("rejects if certificate validation fails", async () => {
      vi.spyOn(Certificate, "create").mockRejectedValueOnce(
        new Error("invalid cert"),
      );

      const agent = await createAgent();

      await expect(agent.call(CANISTER_ID, CALL_FIELDS)).rejects.toSatisfy(
        (error: unknown) => {
  ...
  describe("readState", () => {
    ...
    it("rejects unsupported paths", async () => {
      ...
      await expect(
        agent.readState(CANISTER_ID, { paths: [] }),
      ).rejects.toThrow("Given paths are not supported");
    });
  });
  ...
  describe("sequential execution", () => {
    it("executes calls sequentially through the queue", async () => {
      ...
      const { signer } = createMockSigner();
      (signer.callCanister as any).mockImplementation(
        async () => {
