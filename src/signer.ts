...
import { type PublicKey, type Signature } from "@icp-sdk/core/agent";
import {
  type Channel,
  type JsonRpcError,
  type JsonRpcRequest,
...
export class Signer<T extends Transport = Transport> {
  ...
  async requestPermissions(
    ...
    return this.#rpc({
      ...
      params: scopes,
      encode: z
        .array(z.custom<PermissionScope>())
  ...
  > {
    return this.#rpc({
      ...
      decode: z
        .object({
          accounts: z.array(
            z.object({ owner: zPrincipal, subaccount: zBase64.optional() }),
          ),
        })
  ...
  async requestDelegation(params: {
    ...
    return this.#rpc({
      ...
      encode: z
        ...
        .transform((v) => ({
          publicKey: z.util.uint8ArrayToBase64(
            new Uint8Array(v.publicKey.toDer()),
          ),
          targets: v.targets?.map((t) => t.toText()),
          maxTimeToLive:
            v.maxTimeToLive !== undefined
              ? String(v.maxTimeToLive)
              : undefined,
        })),
      decode: z
        .object({
    ...
    return this.#rpc({
      ...
      encode: z
        ...
        .transform((v) => ({
          ...
          nonce:
            v.nonce !== undefined
              ? z.util.uint8ArrayToBase64(v.nonce)
              : undefined,
        })),
      decode: z.object({
        contentMap: zBase64,
